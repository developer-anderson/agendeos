    ...
    <div class='order'>
        ...
        <ul class='order-list'>
            @foreach($os->servicos as $servico)
                <li>
                    <h4>{{ $servico['nome'] }}</h4>
                    <h5>R$ {{ number_format($servico['valor']/100, 2, ",", ".") }}</h5>
                </li>
            @endforeach
        </ul>
        <h5>Taxa</h5><h4>R$ {{ number_format($os->taxa/100, 2, ",", ".") }}</h4>
        <h5 class='total'>Total</h5><h1>R$ {{ number_format(($os->total+$os->taxa)/100, 2, ",", ".") }}</h1>
    </div>
    <h2>Pagamento</h2>
